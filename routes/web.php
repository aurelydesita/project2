...
Route::middleware(['auth','role:admin'])->group(function () {
    ...
    Route::put('/admin/users/{user}/permissions', [UserController::class,'updateRoles'])->name('users.updateRoles'); 
    Route::get('/admin/users/{user}/permissions/edit', [UserController::class,'edit'])->name('users.permissions.edit');
...
Route::middleware(['auth'])->group(function () {
    ...
    // Export kategori (hanya user yg punya permission kategori_export)
    Route::get('/export/kategori', function () {
        return Excel::download(new KategoriExport, 'kategori.xlsx');
    })->middleware('permission:kategori_export')->name('export.kategori');

    // Export konten
    Route::get('/export/konten', function () {
        return Excel::download(new KontenExport, 'konten.xlsx');
    })->middleware('permission:konten_export')->name('export.konten');

    // Export user (khusus admin + permission user_export)
    Route::get('/export/user', function () {
        return Excel::download(new UserExport, 'user.xlsx');
    })->middleware(['role:admin', 'permission:user_export'])->name('export.user');

    // Export semua data (khusus admin)
    Route::get('/export/all', function () {
        return Excel::download(new MultiSheetExport, 'data_semua.xlsx');
    })->middleware(['role:admin'])->name('export.all');
});
